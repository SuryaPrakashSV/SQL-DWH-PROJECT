...
Script Purpose:
This stored procedure loads data into the 'bze' schema from external CSV files.
...
*/
--BULK LOADING THE CRM TABLES WITH CRM DATA CRM DATA SETS
...
--BULK LOADING THE CRM TABLES WITH ERP DATA ERP DATA SETS
--1
...
PRINT ' >>LOAD DURATION : ' +CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + 'seconds';
print ' >>----------'
SELECT COUNT(*) FROM  bronze.erp_local_a101
